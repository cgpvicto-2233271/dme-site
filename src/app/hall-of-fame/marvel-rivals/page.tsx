...
export default function HallOfFameMarvelRivalsPage() {
  // Pour l’instant il n’y a pas de résultats Marvel Rivals dans achievements,
  // mais on prépare la logique au cas où tu en ajoutes plus tard :
  const filtered = achievements.filter((a) => a.jeu === "marvel-rivals"); // ou tu pourras ajouter un nouveau jeu plus tard
  ...
  return (
    <div className="bg-black text-white">
      ...
      <section className="bg-texture min-h-screen pt-10 pb-16">
        ...
        {/* Liste MR (pour l’instant, vide + message) */}
        <main className="mx-auto w-full max-w-5xl lg:max-w-[102rem] px-4 sm:px-6 pb-16">
          {filtered.length === 0 ? (
            <div className="mx-auto max-w-xl rounded-2xl border border-red-700/70 bg-black/80 px-5 sm:px-6 py-6 text-xs sm:text-sm text-center text-white/80">
              Aucun résultat Marvel Rivals n&apos;est enregistré pour le
              moment.
              <br />
