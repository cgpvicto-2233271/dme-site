...
type LolRank = (typeof LOL_ORDER)[number];
...
function rankToIndexLoL(rank: string) {
  ...
  const idx = (LOL_ORDER as readonly string[]).indexOf(r);
  return idx === -1 ? -1 : idx;
}

function isAtLeastLoL(rank: string, minRank: LolRank) {
...
function trackerRocketSearchUrl(pseudo: string) {
  return `https://tracker.gg/rocket-league/profile/search?query=${encodeURIComponent(pseudo)}`;
}
...
function buildLolPlayers(): Player[] {
  return joueursLolDmeAegis.map((j, idx) => {
    ...
    const isStaffSlot = j.slot === "C" || j.slot === "M";
    const role = isStaffSlot ? undefined : j.slot;

    const rang = isStaffSlot ? "—" : lolPeakToNormalizedRank(j.peak);
...
const JEUX: JeuData[] = [
...

  {
    ...
    rankOptions: [{ id: "all", label: "Tous", predicate: () => true }],
  },

  {
    ...
    players: ROCKET_JOUEURS_GCPLUS.map((pseudo, idx) => ({
      ...
      pseudo,
      rang: "GC+",
      trackers: { tracker: trackerRocketSearchUrl(pseudo) },
    })),
    rankOptions: [{ id: "all", label: "Tous (GC+)", predicate: () => true }],
  },
...
export default function ScoutingClient({ role, discordUrl }: Props) {
  const aAccesInterne = role === "staff"; // (si tu ajoutes coach plus tard, tu peux l'ajouter ici)
  const estLite = !aAccesInterne; // joueur/public/pending -> lite
  const estPendingStaff = role === "pending_staff";
  ...
  const data = useMemo(() => JEUX.find((j) => j.id === jeuActif) ?? JEUX[0], [jeuActif]);
  ...
  const rankPredicate = useMemo(() => {
    return data.rankOptions.find((o) => o.id === filtreRank)?.predicate ?? (() => true);
  ...
  const afficherColonnesStats = !estLite && data.id !== "rocket"; // stats seulement en full
      ...
      <div className="w-full max-w-[1800px] mx-auto px-4">
          ...
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6">
            <div>
              ...
              <p className="mt-3 text-white/75 max-w-3xl">
                {aAccesInterne
                  ? "Mode interne: scouting complet (stats + liens + suivi)."
                  : "Mode public/joueur: scouting simple (liens trackers + roster)."}
              </p>
              ...
              <div className="mt-5 flex flex-wrap gap-3">
                ...
                {aAccesInterne ? (
                  <div className="px-5 py-2 rounded-xl border border-red-600/25 bg-black/25 text-white/80">
                    Acces: <span className="text-red-300 font-semibold">STAFF</span>
                  </div>
                ) : estPendingStaff ? (
                  <a
                    href={discordUrl}
                  ...
                  >
                    Staff: ouvrir un ticket (Discord)
                  </a>
                ) : (
                  <Link
                    href="/connexion"
                    className="px-5 py-2 rounded-xl border border-red-600/45 bg-black/25 hover:border-red-500 hover:text-red-400 transition"
                  >
                    Ajouter mon email (accès)
                  </Link>
                )}
              </div>

              {estPendingStaff ? (
                <div className="mt-4 rounded-2xl border border-white/10 bg-black/20 p-4 text-white/70 text-sm">
                  Tu as demande un acces staff. Va sur le Discord DME et ouvre un ticket pour recevoir
                  les permissions du scouting complet.
                </div>
              ) : null}
            </div>
        ...
        {!estLite ? (
          ...
          <section className="mt-8 rounded-3xl border border-white/10 bg-white/5 backdrop-blur p-6">
            <div className="flex items-end justify-between gap-4 flex-wrap">
              <div>
                <h2 className="text-xl font-bold">Scouting simple</h2>
                <p className="text-white/70 mt-1">
                  Version joueur: roster + liens trackers. Les stats internes sont reservees au staff.
                </p>
              </div>
              <div className="text-sm text-white/60">
                Mode: <span className="text-red-300">Joueur/Public</span>
              </div>
            </div>
          </section>
        )}
        ...
        <section className="mt-8 rounded-3xl border border-white/10 bg-white/5 backdrop-blur p-6">
          ...
          <div className="mt-5 grid lg:grid-cols-12 gap-3">
            ...
            <div className="lg:col-span-4">
              ...
              <select
                value={tri}
                onChange={(e) => setTri(e.target.value as "rang" | "games" | "pseudo")}
                className="mt-2 w-full rounded-2xl border border-white/10 bg-black/25 px-4 py-3 text-sm text-white outline-none focus:border-red-600/35"
              >
                <option value="rang">Rank (desc)</option>
                {!estLite ? <option value="games">Games 7j (desc)</option> : null}
            ...
            <table className="w-full text-sm">
              <thead className="text-white/70">
                <tr className="border-b border-white/10">
                  ...
                  {afficherColonnesRiot ? <th className="py-3 text-left">Riot ID</th> : null}
                    ...
                    <tr key={p.id} className="border-b border-white/10">
                      ...
                      <td className="py-3">
                        <div className="flex flex-wrap gap-2">
                          {/* DPM en full seulement */}
                          {!estLite && p.trackers?.dpm ? (
          ...
          <div className="mt-5 flex flex-wrap items-center justify-between gap-3">
            <div className="text-white/60 text-sm">
              Affiches:{" "}
              <span className="text-white/85 font-semibold">{joueursFiltres.length}</span> /{" "}
              {data.players.length}
            </div>
            ...
            return (
              <circle
                ...
                style={{
                  ...
                  filter:
                    i === 0 ? "drop-shadow(0 0 10px rgba(220,38,38,0.35))" : undefined,
                }}
              />
            );
          })}
