...
const SHEET_ID = "14gjwuQAn4WxDf-YnOHm4BaLI1ktgcZ2tGxb1kLTTuGw";
...
function extractConferences(tsv: string): ConferenceData[] {
  const targon: ConferenceData = { name: "Targon", teams: [] };
  const freljord: ConferenceData = { name: "Freljord", teams: [] };

  const lines = tsv.split("\n").map(normalizeLine).filter(Boolean);

  let currentConference: "Targon" | "Freljord" | null = null;
  ...
  for (const line of lines) {
    ...
    if (lower.includes("targon standings") || lower === "targon") {
      currentConference = "Targon";
      continue;
    }

    if (lower.includes("freljord standings") || lower === "freljord") {
      currentConference = "Freljord";
      continue;
    }
    ...
    if (currentConference === "Targon") {
      targon.teams.push(teamRow);
    } else {
      freljord.teams.push(teamRow);
    }
  }

  return [targon, freljord].filter((conf) => conf.teams.length > 0);
}
  ...
  try {
    ...
    if (conferences.length === 0) {
      return NextResponse.json(
        { error: "Aucune équipe trouvée dans le TSV AEL" },
        { status: 404 }
      );
    }
