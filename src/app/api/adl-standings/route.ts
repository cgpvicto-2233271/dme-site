...
const SHEET_ID = "1wnOHRNQFXaIAp-eloxMmeyM7dt8Mdmyn8csTxt-QB8c";
const SHEET_GID = "1281713505";
...
function extractConferences(tsv: string): ConferenceData[] {
  const ionia: ConferenceData = { name: "Ionia", teams: [] };
  const ixtal: ConferenceData = { name: "Ixtal", teams: [] };

  const lines = tsv.split("\n").map(normalizeLine).filter(Boolean);

  let currentConference: "Ionia" | "Ixtal" | null = null;
  ...
  for (const line of lines) {
    ...
    if (lower.includes("ionia standings") || lower === "ionia") {
      currentConference = "Ionia";
      continue;
    }

    if (lower.includes("ixtal standings") || lower === "ixtal") {
      currentConference = "Ixtal";
      continue;
    }
    ...
    if (currentConference === "Ionia") {
      ionia.teams.push(teamRow);
    } else {
      ixtal.teams.push(teamRow);
    }
  }

  return [ionia, ixtal].filter((conf) => conf.teams.length > 0);
}
  ...
  try {
    ...
    if (conferences.length === 0) {
      return NextResponse.json(
        { error: "Aucune équipe trouvée dans le TSV ADL" },
        { status: 404 }
      );
    }
