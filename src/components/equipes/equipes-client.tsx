      ...
      <Link href={game.href} className="group relative flex h-[520px] w-full flex-col overflow-hidden">
        ...
        <div className="absolute inset-0">
          <Image
            ...
            quality={90}
            className="object-cover object-center transition-all duration-700 group-hover:scale-[1.05] group-hover:brightness-75"
          />
          <div className="absolute inset-0 bg-gradient-to-t from-[#080808] via-[#080808]/50 to-transparent" />
          {/* Extra darkness on bottom for text legibility */}
        ...
        {/* Index number */}
        <div className="absolute right-5 top-5 font-mono text-[9px] font-black tracking-[0.3em] text-white/20">
          {String(index + 1).padStart(2, "0")}
        </div>

        {/* Red glow on hover */}
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_80%_40%_at_50%_100%,rgba(220,38,38,0.12),transparent)] opacity-0 transition-opacity duration-500 group-hover:opacity-100" />

        {/* Content */}
        <div className="absolute inset-x-0 bottom-0 p-7">
          <p className="mb-2 text-[9px] font-black uppercase tracking-[0.38em] text-red-500/70">{game.sub}</p>
          <h3 className="font-display mb-5 text-[clamp(1.4rem,3vw,1.9rem)] uppercase leading-none text-white">{game.label}</h3>
          <div className="flex items-center justify-between">
            ...
            <div className="flex h-9 w-9 items-center justify-center border border-white/[0.1] text-white/35 transition-all duration-300 group-hover:border-red-600/50 group-hover:text-red-500 group-hover:bg-red-600/8">
              →
            </div>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#080808] text-white">
      ...
      <section className="relative overflow-hidden border-b border-white/[0.05] pb-20 pt-28">
        ...
        <div aria-hidden className="pointer-events-none absolute right-0 top-1/2 -translate-y-1/2 select-none font-display uppercase leading-none text-white/[0.025]" style={{ fontSize: "clamp(10rem,22vw,20rem)" }}>DME</div>
          ...
          <div className="flex flex-col gap-10 lg:flex-row lg:items-end lg:justify-between">
            ...
            <motion.div className="flex divide-x divide-white/[0.07] border border-white/[0.08]" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.4, ease }}>
              {[
                ...
                <div key={s.labelFr} className="px-4 py-4 text-center sm:px-8 sm:py-5">
                  <p className="font-display text-[1.6rem] font-black tabular-nums leading-none text-white sm:text-[2rem]">{s.val}</p>
                  <p className="mt-1.5 text-[7px] font-black uppercase tracking-[0.25em] text-white/30 sm:text-[8px] sm:tracking-[0.35em]">{t(s.labelFr, s.labelEn)}</p>
                </div>
              ))}
            </motion.div>
          </div>
      ...
      <section className="mx-auto max-w-[120rem] px-6 py-16 sm:px-10">
        <div className="mb-10 flex items-center gap-5">
          ...
          <span className="font-mono text-[9px] font-black uppercase tracking-[0.4em] text-white/22">{t("Sélectionne un jeu", "Select a Game")}</span>
        ...
        <div className="mx-auto max-w-[120rem] px-6 sm:px-10">
          <motion.div className="mb-14" initial={{ opacity: 0, y: 24 }} animate={pillarInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, ease }}>
            <div className="mb-4 flex items-center gap-4">
              <div className="h-px w-8 bg-red-600" />
              <span className="font-mono text-[9px] font-black uppercase tracking-[0.45em] text-white/30">02 — {t("Identité", "Identity")}</span>
            </div>
          ...
          <div className="grid gap-[1px] bg-white/[0.05] sm:grid-cols-3">
            {pillars.map((b, i) => (
              <motion.div key={b.num}
                className="group relative overflow-hidden bg-[#060606] px-8 py-12"
                initial={{ opacity: 0, y: 32 }} animate={pillarInView ? { opacity: 1, y: 0 } : {}}
              ...
              >
                ...
                <div className="absolute inset-0 bg-gradient-to-b from-red-900/0 to-transparent transition-all duration-500 group-hover:from-red-900/[0.06]" />
                <p className="relative mb-6 font-mono text-[9px] font-black tracking-[0.3em] text-red-600/40">{b.num}</p>
      ...
      {/* ── PARTENAIRES ── */}
      {track.length > 0 && (
        <div className="overflow-hidden border-y border-white/[0.04] bg-[#050505] py-7">
      ...
      <section ref={ctaRef} className="border-t border-white/[0.04] py-28">
        <div className="mx-auto max-w-[120rem] px-6 sm:px-10">
          <motion.div className="relative overflow-hidden" initial={{ opacity: 0, y: 32 }} animate={ctaInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.7, ease }}>
            <div aria-hidden className="pointer-events-none absolute right-0 top-1/2 -translate-y-1/2 select-none font-display uppercase leading-none text-white/[0.02]" style={{ fontSize: "clamp(8rem,18vw,16rem)" }}>DME</div>
            <div className="relative">
              <div className="mb-6 flex items-center gap-4">
                <div className="h-px w-8 bg-red-600" />
                <span className="font-mono text-[9px] font-black uppercase tracking-[0.45em] text-white/28">{t("Recrutement ouvert", "Open Recruitment")}</span>
              </div>
              <h2 className="font-display mb-10 uppercase leading-[0.9]" style={{ fontSize: "clamp(2.5rem,6vw,5.5rem)" }}>
